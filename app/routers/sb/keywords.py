...
from app.models.sb import SBKeyword
from app.routers.sp._helpers import apply_id_filter, apply_state_filter, paginate
from app.schemas.sb_v4 import SBKeywordCreate
from app.services.ids import numeric_id

router = APIRouter(prefix="/sb/v4/keywords", tags=["sb-keywords"])


def _to_dict(k: SBKeyword) -> dict[str, Any]:
    return {
...
def list_keywords(
    ...
    rows = db.query(SBKeyword).filter(SBKeyword.profile_id == auth.profile_id).all()
...
def create_keywords(
    ...
    for i, item in enumerate(items):
        try:
            data = SBKeywordCreate.model_validate(item)
...
def update_keywords(
    ...
    for i, item in enumerate(items):
        ...
        k = (
            db.query(SBKeyword)
            .filter(SBKeyword.profile_id == auth.profile_id, SBKeyword.keyword_id == kid)
            .first()
        )
        if k is None:
            error.append({"index": i, "code": "NOT_FOUND", "details": f"Keyword {kid} not found"})
            continue
        if "state" in item:
            k.state = str(item["state"]).upper()
        if "bid" in item:
            k.bid = float(item["bid"])
...
def delete_keyword(
    ...
    k = (
        db.query(SBKeyword)
        .filter(SBKeyword.profile_id == auth.profile_id, SBKeyword.keyword_id == keyword_id)
        .first()
    )
    if k is None:
